...
describe('game menu plugin', () => {
  ...
  it('opens ranking from the main menu and keeps the overlay nested', async () => {
    const world = await boot();
    const ada = join(world, 'Ada');
    world.handleMenuAction(ada.player, { type: 'menu_action', action: 'open', screen: 'root' });
    world.handleMenuAction(ada.player, { type: 'menu_action', action: 'open', screen: 'rating' });
    expect(lastOf<ServerMenuMessage>(ada.sink, 'menu')?.screen).toBe('rating');
    world.handleMenuAction(ada.player, { type: 'menu_action', action: 'back' });
    expect(lastOf<ServerMenuMessage>(ada.sink, 'menu')?.screen).toBe('root');
  });

  it('opens a clan card from the menu list without closing the overlay', async () => {
    const world = await boot();
    const ada = join(world, 'Ada');
    ...
    let clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.screen).toBe('create');
    ...
    clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.screen).toBe('ranking');
    expect(clan?.rankingSort).toBe('kills');
    world.handleClanAction(ada.player, { type: 'clan_action', action: 'set_ranking_sort', sort: 'money' });
    clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.rankingSort).toBe('money');
    ...
    clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.screen).toBe('ranking');
    const closedMenu = lastOf<ServerMenuMessage>(ada.sink, 'menu');
    expect(closedMenu?.screen === 'closed' || closedMenu === undefined || closedMenu?.screen === 'clans' || closedMenu?.screen === 'root').toBe(true);
    expect(clan?.screen).not.toBe('closed');
  });

  it('ignores select_icon on the clan list and still opens a card by row id', async () => {
    const world = await boot();
    const ada = join(world, 'Ada');
    world.economy.deposit(ada.player.id, 20_000, 'ADMIN_GIVE');
    world.handleMenuAction(ada.player, { type: 'menu_action', action: 'clans_create' });
    ...
    expect(clan?.screen).toBe('ranking');
    const clanId = clan?.clans[0]?.clanId;
    world.handleClanAction(ada.player, { type: 'clan_action', action: 'select_icon', icon: 'crown' });
    clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.screen).toBe('ranking');
    expect(clan?.screen).not.toBe('create');
    expect(clan?.screen).not.toBe('closed');
    world.handleClanAction(ada.player, { type: 'clan_action', action: 'select_clan', clanId });
    clan = lastOf<ServerClanMessage>(ada.sink, 'clan');
    expect(clan?.screen).toBe('card');

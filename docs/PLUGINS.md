...
- Successful `/clan add` sends a one-shot system chat to the target: `Игрок <owner> пригласил вас в клан <name>. Используйте /clan accept…`. Duplicate invite is an error (`Игрок уже получил приглашение`) and does not resend. Requests tab also invites by nickname (offline stored identities allowed); `/clan add` picker stays online-only.
...
- Persistence: `plugin-data/clans/clans.json` via `JsonFileStore` (includes `roles`). Totals and kills are not stored on the clan. Load migrates missing roles (owner→leader, others→member).
- Protocol: client `clan_action` (intent only, including `invite_by_name` / `set_member_sort` / `promote_veteran` / `transfer_leader` / `friends_request`), server `clan` (paged snapshot + `playerCard`). Search patches the list in place so the input keeps focus and caret.
...
- In-game menu Кланы opens this same GUI (`open_my_clan` / `open_clan_list` / `open_create_clan`). Messages include `source: 'menu'`; ranking/card/create ← returns to the menu Clans hub instead of closing everything. Menu **Рейтинг** is a nested `menu` screen (`rating_set` / `rating_page`), not a second clan overlay.

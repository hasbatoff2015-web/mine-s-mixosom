...
- GUI reuses Auction House inventory chrome (`mc-backdrop` / `mc-panel` / `mc-ah-btn` / close × / E to close unless a field is focused). Ranking uses a fixed-width rank column (CSS cups for #1–#3, `#N` after that) then `[icon] name`, compact МК, `n/20`. Clan card has back ← (does not close the GUI). An active invitation to that clan shows **Вступить в клан** (`joinState: invited`); the server re-checks the invitation on click.

...
- Глава ставит/меняет базу с текущей позиции (блок под ногами → `diamond_block`). Bedrock не заменяется: якорь на блок выше. Пересечение с любым существующим claim запрещено, в том числе со своими личными. Cooldown 24ч на клан, только после успешной установки.

...
## Последний проход: Repair potion — 2026-09-18

- Consumable `potion_repair` / **Зелье починки**: выпивается как существующие зелья (32 tick, `alwaysEdible`, возвращает `glass_bottle`), без status effect. На сервере и в Singleplayer восстанавливает **50% потерянной** remaining durability у всех предметов игрока с `definition.durability` (hotbar, main, armor, offhand). Целые предметы не меняются; durability не превышает max; полностью починенный stack снова без поля `durability`. Рецепта нет. Buyer example: `potion_repair` за **500** МК (`BUYER_EXAMPLE_REPAIR_POTION_*`). Визуал — тот же bottle+overlay, тинт teal `[18, 181, 164]`.

...
- [ ] Последний visual QA: крупные и не тёмные special icons, Creative scroll, ladder, inventory/pause close возвращает pointer lock (checklist в `docs/reports/2026-08-22_pause-resume-pointer-lock.md`), затем подготовка PR #2 к merge в main. Не начинать chest GUI / furnace GUI / recipe book / creative flight в том же pass.

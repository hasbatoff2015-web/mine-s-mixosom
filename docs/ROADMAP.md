...
- [ ] Auction House / Trader remain later; they should call EconomyService, not a second wallet.
...
- [x] Create cuboid block-claims on `blockPlaced` (radii 10/20/30 on X, Y and Z) inside the existing Claims plugin.
...
- [x] Show existing `claim_boundary` wires on successful block-claim place and on overlap deny (existing claims, not the attempted volume).
- [x] Block-claim volume is cubic (±radius on Y, same as X/Z); load rebuilds volume from `Claim.anchor` so old full-height saves are not kept.
...
- [x] Keep V2 FIFO / `ackCommandSeq` / prediction / serverTick interpolation / protocol 3.
- [x] Keep Farming protocol `hydrated`/`age` and server/SP `FarmingSystem`.
- [x] Replace latest-input chase/snap characterization tests with the V2+Farming contract.
- [ ] Owner two-client walk/sprint/jump/remote/farming QA on a new world.
...
- [x] Farming V1 is on `main`; this union keeps it.
- [ ] Owner two-client live QA on Farming+V2 (new world).

...
Sword use вычисляется на fixed tick из selected sword + held use + active gameplay + alive (`isSwordItem`); release/switch/death/overlay/pause сбрасывают. **SP `tickPlayers` и Anarchy `tickOnline` оба вызывают `Game.syncLocalCombatUse`** (`setHeldItem` → `setOffhand` → `combat.updateUse`). Anarchy раньше пропускал `updateUse`: `movementDuringItemUse` уже тормозил из `input.using`, а `combat.swordBlocking` оставался `false`, поэтому first/third-person overlay не запускался. Game масштабирует forward/strafe ×0.2 и отключает sprint. Visual blocking — render-only overlay: `blockingProgress` 0→1 за 0.1 с в `PlayerVisualAnimator` / `FirstPersonRenderer` / `PlayerVisual`, поверх idle `/moveitems` и `FIRST_PERSON_SPRITE_POSE`. First-person: `applyItemViewTransform` (idle/QA), затем `applyFirstPersonSwordBlockingOverlay` на том же `mainModel`. Third-person: stored `/moveitems` base + extra local TRS на `heldModel`. Remote клиенты читают уже существующий `presentation.swordBlocking` через `RemotePlayerView` → `PlayerVisual.update`. Combat serialization не содержит blocking/timers; world save schema прежняя.

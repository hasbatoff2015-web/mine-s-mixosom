...
`itemRenderProfiles` разделяет **pose category** (`block` / `torch` / `generated` / `handheld` / `bow` / `shield`) и **mesh kind** (`itemVisualKind`: `block-cube` / `special-torch` / `generated`). `ItemVisualFactory` остаётся единственным adapter: cube block item получает UV-cube из шести block textures; torch/redstone torch — `createTorchItemGeometry` с тем же `TORCH_WIDTH`/`TORCH_HEIGHT`/`TORCH_TEXTURE_UV`, что и world torch; любой non-block item проходит через `GeneratedItemGeometry`. Она читает alpha mask, строит front/back только по opaque row spans (силуэт, не полный прямоугольник) и граничные side strips, объединяя соседние spans вместо кубика на каждый пиксель. Глубина `0.10`. Геометрии, материалы и textures, включая bow pull variants, загружаются заранее и кэшируются по item/block key; во frame loop меняются только transform или ссылка на уже готовый variant mesh. First-person generated/handheld yaw достаточен, чтобы читалась толщина, а не billboard.

...
`HologramRenderer` draws a `THREE.Group` with a black background **plane** and a text **plane** (canvas without baked fill). Billboard copies `camera.quaternion` each frame. Fixed sets quaternion from stored yaw only — not a Sprite, not `lookAt`. Hit AABB uses `hologramWorldSize` (max of text sprite and background when enabled).

...
`itemRenderProfiles` — data-only слой классификации `block/generated/handheld/bow/shield`. `item/handheld` — semantic parent `item/generated`: та же sprite geometry, без отдельной 3D-геометрии инструментов. First-person generated/handheld/bow всё ещё используют временный shared pose `[0.50, -0.56, -0.82]`, Euler `[0, 0, 14]°`, `scale 0.85` (final Three.js uniform, не `0.68 * multiplier`). Это face-on calibration, не vanilla pipeline: JSON `rotation [0,-90,25]` — реальный display rotate (front +Z → camera −X), а не «basis conversion», и в production пока не применяется. `front·look ≈ 0` после Ry(−90) не скрывает лицо: item справа, `front·toCamera > 0`. Канонический adapter — `heldItemVanillaTransform.ts` (`T_hand * T_disp/16 * Rx * Ry * Rz * S`); 1.9 и 1.21.8 idle RH совпадают; `T(-0.5)` не нужен, геометрия уже centered. Production pose не переключали. Dev `held*` остаётся QA override этого временного pose. `?qaItem=` по умолчанию изолирует предмет (`qaView=front|back|left|right`); `qaView=held` — first-person; `qaView=held&pose=idle` печатает FOV/aspect, matrices, axis stages, silhouette landmarks и F2 2048×1152 comparison (камера сравнения — F2 16:9 FOV70). `qaSideDebug=1` красит стороны. `ItemVisualFactory` — общий adapter: cube block item из atlas, sprite items (включая held torch) через `GeneratedItemGeometry`. Held torch не переиспользует placed cuboid; projectile `ArrowVisualFactory` не используется для held arrow.

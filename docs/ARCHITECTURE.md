...
Каждый chunk хранит два `Uint8Array` света. Sky light заполняется сверху вниз по столбцу и коротко растекается по соседям; block light flood идёт от emissive блоков (torch 14, redstone torch 7, lava 15) с лимитом `8192` узлов. `getChunk` генерирует terrain **без** lighting; `ensureChunkLighting` — отдельная job stage. Одиночная смена блока: если opacity+emission не менялись, lighting пропускается; иначе локальные sky-столбцы (radius 4) и/или block flood по AABB. LightEngine **не** ставит `chunk.dirty` на каждый light write. Geometry dirty: mutation chunk + X/Z boundary neighbors (`neighborMeshOffsets`); emission (torch/furnace) дополнительно dirty overlapping light AABB. Массовые записи идут через `VoxelWorld.applyBlockBatch` с `pendingMesh` Set (dedupe) и optional `deferLighting` (один `pendingLight` на frame, merge AABB). Cube faces пишут per-vertex `skyLight`/`blockLight` как среднее четырёх клеток у вершины (smooth lighting lite), плюс `faceShade` / `emissionLight` и biome tint в `color`. Chunk materials — `MeshBasicMaterial` с `onBeforeCompile`: shader делает `max(sky^γ * daylight, torchWarm * block)` и **не** применяет Lambert N·L. World entities используют тот же `composeWorldLight` через `sampleEntityLight` на simulation tick. Тёплый цвет факела живёт только в block-light contribution, daylight/sky остаётся нейтральным.

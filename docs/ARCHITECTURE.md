...
`PlayerVisual` owns separate skin base and outer materials while all of them reference the single texture handle acquired from `MinecraftSkinRegistry`. Base is always an opaque cutout (`alphaTest=0.01`, `transparent=false`, `depthTest=true`, `depthWrite=true`). A data-only `outerLayerAlpha` flag in the built-in descriptor enables blending only for production skins whose used outer UV islands contain alpha 1..254; all other outer layers remain opaque cutouts. Translucent outer materials keep `depthWrite=true` so an outer shell cannot reappear through later opaque armor. Appearance changes update the maps/transparency on the existing materials and release the previous texture handle; model changes rebuild meshes against the same material objects.

Skin parts share the armor priority convention: head/body `0`, right arm/leg `1`, left arm/leg `2`. Base `renderOrder` is `0 + priority`, outer is `10 + priority`; outer per-priority materials use no polygon offset at priority 0, then factor/units `-1/-1` and `-2/-2`. Armor remains base `20/21/22` and leather overlay `30/31/32`, establishing `skin base < skin outer < armor base < armor overlay`. Skin base has no polygon offset. This is a render/depth policy only: 64×64 UV semantics, Classic/Slim dimensions, pivots, positions, scale, hat inflate 0.5 px and other outer inflate 0.25 px are unchanged.

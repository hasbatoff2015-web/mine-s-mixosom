...
Remote/third-person held items **не** читают `FIRST_PERSON_SPRITE_POSE`. `RemotePlayerView` вызывает `PlayerVisual.setHeldItem` → `ItemVisualFactory.createItemModel` → category defaults из `src/rendering/player/thirdPersonHeldItem.ts` (те же числа, что раньше были inline в `applyHeldItemTransform`: block scale `0.24`, handheld `0.55`, generated `0.40`, bow `0.46`). DEV-маршрут `/moveitems` (`src/dev/MoveItemsHarness.ts`) открывает маленький `VoxelWorld` + `WorldRenderer` и тот же `PlayerVisual`; live pos/rot/scale идут через `PlayerVisual.applyHeldItemCalibration` и **не** записываются в production. First-person calibrator `?qaItem=` / `HeldItemPosePanel` остаётся отдельным.

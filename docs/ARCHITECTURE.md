...
- World events are `WorldEventsManager` + builtin `world-events`. One active `resource_chest` at a time. Persistence is `plugin-data/world-events/state.json`. Event-area voxels are explosion-protected through `ServerGameplay.isExplosionProtected` (same `canDestroy` path as regular claims).

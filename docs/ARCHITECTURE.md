...
Selection outline — тот же `LineSegments` в `WorldRenderer`. `selectionBoxesForBlock()` строит oriented boxes из фактической special geometry (cube / torch / button / lever / plate / wire / door / ladder / cross / fire / stairs / slab / chest); геометрии кэшируются по shape key. Voxel raycast пересекает `blockCollisionBoxes` для solid блоков, поэтому пустая половина slab/stair не выделяется.
...
`MinecartManager` — bounded rail vehicle (cap 16). Visual — open-top 3D group (`minecartGeometry.ts`, `entity/minecart` exterior, darker inner/floor, optional TNT cube). Движение не через player collision: `railPath.ts` семплирует current rail + immediate next cell (straight / quarter-turn / ascending). Rider W/S проецирует look на tangent, cap `WALK_SPEED`, accel ~0.5 с, coast friction, slope gravity. A/D зарезервированы под junctions и не сдвигают cart с рельс. Player push — AABB overlap, только компонента на tangent. TNT Use ставит variant `tnt` (общий movement, не rideable); Flint 80-tick fuse; Fire Arrow — immediate `ExplosionQueue` с TNT power/radius 4. Isolated rail orientation = player look axis; EW family использует `railTextureYaw = π/2`. Сериализация — optional `minecarts` в schema 1 (position, velocity, yaw, variant, fuse). Это practical approximation, не vanilla minecart physics.

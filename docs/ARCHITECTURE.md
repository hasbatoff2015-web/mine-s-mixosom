...
`itemRenderProfiles` разделяет **pose category** (`block` / `torch` / `generated` / `handheld` / `bow` / `shield`) и **mesh family** (`itemVisualFamily`: `block-cube`, `torch`, `door`, `lever`, `button`, `pressure-plate`, tools/weapons, resources, food, armor, `generic-fallback`). `ItemVisualFactory` остаётся единственным adapter. Cube block item — atlas UV-cube; torch/door/button/plate — atlas cuboids из `specialBlockGeometry`; все остальные held/dropped items — reusable low-poly parts из `ItemFamilyGeometry` с palette materials (handle/head, cover/pages, …). Геометрия кэшируется на family/part, материал — на цвет или atlas layer. `generic-fallback` только для неизвестных будущих item ID. Pixel silhouette extrusion (`GeneratedItemGeometry`) удалён. Лук — группа grip/limbs/string; `FirstPersonRenderer` позует draw geometrically, не сменой плоской texture. Щит клеит `item/shield` atlas на plate cuboid. Во frame loop меняются только transform и bow limb/string pose.

`FirstPersonRenderer` владеет отдельными scene и perspective camera. После world render `Game` очищает только depth buffer и рисует руку/предмет поверх мира; renderer info сбрасывается один раз перед обоими проходами, поэтому F3 учитывает полную сцену. Textured Steve arm — UV-cuboid и показывается только при пустом main slot; при любом held item она скрыта. Frame state содержит движение, землю, sprint, mining, swing, food/bow progress и shield state. Bow draw гнёт limbs и двигает string по `bowProgress` (`ItemVisualFactory.applyBowDraw`); FOV/slowdown остаются прежними. Каждая поза вычисляется от неизменяемого preset, а не от предыдущего кадра, что исключает накопление transform drift. Модель меняется только при смене item id (лук не свопает mesh по pull stage).

...
PLAYING job fairness (`src/world/streamingScheduler.ts`): budgets **не** поднимались (`WORLD_JOB_BUDGET_MS = 4`, light slice 2 ms). PLAYING generate и mesh **не** идут в одном кадре. После gen-only кадра `shouldDeferGenerateForMesh` пропускает следующий generate, если есть любой ready mesh. PLAYING `meshLimit = 1`. Distant chunks (chebyshev ≥ 3) mesh без 4-corner AO (`CHEAP_VERTEX_LIGHT_CHEBYSHEV`). Nearby full AO fills an 18×18×H packed light+occlusion cache before the voxel scan (`neighborhoodLightCache`, default on for `vertexLight: 'full'`). Priority: ring + age + movement-ahead + distanceSq. `discardObsoletePendingMesh` снимает `pendingMesh` вне wanted mesh radius. Mesh lane по-прежнему `continue` мимо blocked head.

...
- `?qaItem=iron_pickaxe&qaView=held&pose=idle` — first-person; overlay печатает camera FOV/aspect, item local/world/modelView, projected `screen01` reference points и proposed vanilla idle matrix (не applied). Residual idle bob заморожен. `held*` knobs работают только здесь;

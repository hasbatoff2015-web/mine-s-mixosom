...
/** Shared bilinear + sealed-corner AO. Used by per-vertex sampling and the 3×3 face prototype. */
export function shadePackedQuad(
  a: number, b: number, c: number, d: number,
  fu: number, fv: number, anchor: number,
  out: SurfaceLight,
): SurfaceLight {
...
export function surfaceSamplePlane(
  px: number, py: number, pz: number,
  nx: number, ny: number, nz: number,
): { axis: 0 | 1 | 2; plane: number; i: number; j: number; fu: number; fv: number } {
  ...
  return { axis, plane, i, j, fu: u - i, fv: v - j };
}
...
export function sampleSurfaceVertexLight(
  ...
  const sample = surfaceSamplePlane(px, py, pz, nx, ny, nz);
  const { axis, plane, i, j, fu, fv } = sample;
  ...
  return shadePackedQuad(a, b, c, d, fu, fv, anchor, out);
}

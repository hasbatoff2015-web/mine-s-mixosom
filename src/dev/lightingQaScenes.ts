...
export type LightingQaScene = 'room' | 'closed' | 'hole' | 'cave' | 'forest' | 'sources' | 'high';

/** Shared deterministic geometry for CPU regressions, benchmarks and the existing DEV viewer. */
export function createLightingQaScene(kind: LightingQaScene, WorldType = VoxelWorld, ChunkType = Chunk,
  floorY = kind === 'high' ? 192 : 39): VoxelWorld {
  ...
  for (let cz = -1; cz <= 2; cz += 1) {
    for (let cx = -1; cx <= 2; cx += 1) {
      ...
      if (floorY > 39) {
        chunk.blocks.fill(BlockId.Stone, floorY * CHUNK_SIZE * CHUNK_SIZE, (floorY + 1) * CHUNK_SIZE * CHUNK_SIZE);
        chunk.occupancyTop = floorY;
      }
  ...
  const put = (x: number, y: number, z: number, block: BlockId): void => {
    world.getChunk(Math.floor(x / CHUNK_SIZE), Math.floor(z / CHUNK_SIZE), false)!
      .set(((x % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE, y + floorY - 39, ((z % CHUNK_SIZE) + CHUNK_SIZE) % CHUNK_SIZE, block);
  };
  if (kind === 'forest') {
    ...
    if (kind === 'room' || kind === 'cave' || kind === 'sources' || kind === 'high') {
...
export function lightingQaOpening(open: boolean, floorY = 39): BlockMutation[] {
  const cells: BlockMutation[] = [];
  for (let z = 4; z <= 27; z += 1) for (let y = 40; y <= 46; y += 1) {
    cells.push({ x: 2, y: y + floorY - 39, z, block: open ? BlockId.Air : BlockId.OakPlanks });
  }
  return cells;
}

export function lightingQaRoofHole(open: boolean, floorY = 39): BlockMutation[] {
  return [{ x: 7, y: floorY + 8, z: 16, block: open ? BlockId.Air : BlockId.OakPlanks }];
}

export function lightingQaSkyLine(world: VoxelWorld, floorY = 39): number[] {
  return Array.from({ length: 26 }, (_, i) => world.skyLightAt(3 + i, Math.min(floorY + 4, WORLD_HEIGHT - 1), 16));
}
